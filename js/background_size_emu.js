...
(function () {
	...
	BgSzEmu.prototype.scanElems = function () {
		if (!BgSzEmu.prototype.IsIE() || !BgSzEmu.prototype.IsBadIE()) return;
		if (document.body) {
			...
			if (!BgSzEmu.prototype.elemsOnPrevCheck) {
				...
				for (var i = 0; i < curr_elems.length; i++)
					if (BgSzEmu.prototype.isObjectInArray(curr_elems[i], BgSzEmu.prototype.elemsOnPrevCheck)) {
						if (!curr_elems[i].junkData) continue;
						var available_size = BgSzEmu.prototype.getAvailableAreaSizeIn(curr_elems[i]);
						if (curr_elems[i].junkData.lastSize && (curr_elems[i].junkData.lastSize.width != available_size.width || curr_elems[i].junkData.lastSize.height != available_size.height)) BgSzEmu.prototype.fixBgFor(curr_elems[i]);
					}
					else {
						var curr_bg_img = BgSzEmu.prototype.getCSSPropertyValue(curr_elems[i], "background-image", "backgroundImage");
						if (curr_bg_img && !curr_elems[i].junkData) BgSzEmu.prototype.fixBgFor(curr_elems[i]);
					}
				BgSzEmu.prototype.elemsOnPrevCheck = curr_elems.slice(0);
			}
		}
	...
	BgSzEmu.prototype.fixBgsRecursiveIn = function (start_elem) {
		...
		if (bg_sz && bg_sz.toLowerCase() != "auto auto") BgSzEmu.prototype.fixBgFor(curr_elem);
		for (var i = 0; i < curr_elem.children.length; i++) BgSzEmu.prototype.fixBgsRecursiveIn(curr_elem.children[i]);
	};
	...
	BgSzEmu.prototype.handlePropertyChange = function () {
		...
		if (evt.propertyName == "onpropertychange" || !elem) return;
		if (evt.propertyName == "style.backgroundImage") {
			...
			else BgSzEmu.prototype.replaceBgImgFor(elem);
		}
		else if (BgSzEmu.prototype.startsWith(evt.propertyName, "style.background")) BgSzEmu.prototype.replaceBgImgFor(elem);
	};
	BgSzEmu.prototype.replaceBgImgFor = function (elem) {
		...
		if (e_avl_sz.width == 0 || e_avl_sz.height == 0) return;
		...
		{
			if (prev_backgroundImage == BgSzEmu.prototype.genericEmptyBG) BgSzEmu.prototype.fixBgFor(elem);
			else BgSzEmu.prototype.getImgNaturalSizeAndPassToCallback(elem, prev_backgroundImage, BgSzEmu.prototype.continueBgReplaceFor);
		}
		if (prop_change_removed) elem.onpropertychange = BgSzEmu.prototype.handlePropertyChange;
	};
	BgSzEmu.prototype.continueBgReplaceFor = function (elem, prev_backgroundImage, img_natural_size) {
		...
		if ("background" in elem) elem.background = BgSzEmu.prototype.genericEmptyBG;
		...
		if (!stylePosition || stylePosition == "static") elem.style.position = "relative";
		if (!styleZIndex || styleZIndex == "auto") elem.style.zIndex = 0;
		...
		var junkData = {
			orig_bgImg: prev_backgroundImage
			, orig_pos: prev_position
			, orig_zInd: prev_zIndex
			, inner_div: div
			, inner_img: img
			, inner_img_nat_size: img_natural_size
		};
		elem.junkData = junkData;
		div.appendChild(img);
		if (elem.firstChild) elem.insertBefore(div, elem.firstChild);
	...
	BgSzEmu.prototype.fixBgFor = function (elem) {
		...
		if (junkData) {
			...
			if (bg_sz == "cover" || bg_sz == "contain") {
				if ((bg_sz == "cover" && divRatio > imgRatio) || (bg_sz == "contain" && imgRatio > divRatio)) {
					...
					if (elem_bg_pos.v_pos.is_percents) new_img_top = Math.floor((div_height - new_img_height) * elem_bg_pos.v_pos.value) + "px";
				}
				else {
					new_img_height = div_height;
					new_img_width = new_img_height * imgRatio;
					if (elem_bg_pos.h_pos.is_percents) new_img_left = Math.floor((div_width - new_img_width) * elem_bg_pos.h_pos.value) + "px";
				}
				...
				if (t_width.toLowerCase() == "auto" && t_height.toLowerCase() == "auto") {
					...
					if (!width_to_set || width_to_set < 1) width_to_set = 1;
					...
					if (!height_to_set || height_to_set < 1) height_to_set = 1;
		...
		else if (bg_sz) BgSzEmu.prototype.replaceBgImgFor(elem);
	};
	BgSzEmu.prototype.parseBgPosVal = function (word) {
		...
		else if (BgSzEmu.prototype.endsWith(word, "%")) return {
			value: (word.substr(0, word.length - 1) / 100)
			, is_percents: true
		};
		return {
			value: word
	...
	BgSzEmu.prototype.getPurePathFrom = function (str_path) {
		...
		if (final_str.substring(0, ("url(").length) == "url(") {
			...
			if (final_str.lastIndexOf(")") == final_str.length - 1) final_str = final_str.substr(0, final_str.length - 1);
		}
		return final_str;
	};
	BgSzEmu.prototype.getElemBgPos = function (elem) {
		var splitted_pos = Array(BgSzEmu.prototype.getCSSPropertyValue(elem, "background-position-x", "backgroundPositionX"), BgSzEmu.prototype.getCSSPropertyValue(elem, "background-position-y", "backgroundPositionY"));
		var h_pos_ = (splitted_pos[0] ? BgSzEmu.prototype.parseBgPosVal(splitted_pos[0]) : {
			value: "0"
			, is_percents: true
		});
		var v_pos_ = (splitted_pos[1] ? BgSzEmu.prototype.parseBgPosVal(splitted_pos[1]) : {
			value: "0"
			, is_percents: true
		});
		return {
	...
	BgSzEmu.prototype.elemCanHaveDivAsChildren = function (elem) {
		...
		try {
			...
			if (BgSzEmu.prototype.isObjectInArray(div, elem.children)) elem.removeChild(div);
		}
		return check_result;
	};
